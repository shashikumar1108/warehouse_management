...
class Order_model extends CI_Model {
   ...
   public function getWarehouseSuppliers($warehouse_id = null)
   {
      ...
      public function get_vendor_quotation($quotation_id){
         return $this->db->select('*')->from('vendor_quotation')->where('quotation_id',$quotation_id)->get()->result_array();
      }
      ...
      public function get_quotation_product_price($vendor_quote_id){
      ...
      public function get_order_quotation_table_list($payload = array(), $proposal_status = 4, $suppiler_id = 0){
      ...
      public function get_single_order_quotation_details($quotation_id,$proposal_status, $vendor_quote_id = 0){
      ...
      public function get_order_price_details($quotation_id,$proposal_status, $vendor_quote_id = 0){

         $this->db->select('q.quotation_id,q.ref_number,q.description,qp.*,p.name as product_name,
            vq.vendor_quote_id,vqd.vqd_id,vqad.*');
         $this->db->from('quotation q');
         $this->db->join('quotation_products qp','q.quotation_id=qp.quotation_id');
         $this->db->join('vendor_quotation vq','q.quotation_id=vq.quotation_id');
      ...
      // Order payment list
      public function getOrderPaymentList($payload = array(),$supplier_id = 0){

         $order_column = array("q.quotation_id","q.ref_number","w.name","i.paid_amount",
            "i.pending_amount","i.grand_total","i.invoice_status","q.quotation_id");
         ...
         $select = 'q.*,w.name as warehouse_name,vq.vendor_quote_id,i.*,vq.supplier_id';
         $this->db->select($select)
            ->from('invoice i')
            ->join('vendor_quotation vq','vq.vendor_quote_id=i.vendor_quote_id and i.quotation_id = vq.quotation_id')
            ->join('quotation q','q.quotation_id=i.quotation_id')
            ->join('warehouse w','w.id=q.warehouse_id')
         ...
         $this->db->select('q.quotation_id')
            ...
            // ->join('vendor_quotation vq','q.quotation_id=vq.quotation_id')
            // ->join('warehouse w','w.id=q.warehouse_id')
            // ->join('invoice i','q.quotation_id=i.quotation_id and vq.vendor_quote_id = i.vendor_quote_id')
            ->where(array(
               'q.approval_status' => 'approved',
               'q.delete_status' => '0',
               'vq.supplier_id' => $supplier_id,
         ...
         if( isset($payload['search']['value']) ){
            $this->db->like('q.ref_number', trim($payload['search']['value']));
            // $this->db->or_like('i.grand_total', trim($payload['search']['value']));
         }
