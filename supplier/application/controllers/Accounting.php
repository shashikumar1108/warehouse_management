...
class Accounting extends CI_Controller {
    ...
    public function getPaymentTableList(){
        // echo '<pre>';print_r($_POST);exit;

        $products = $this->order_model->getOrderPaymentList($_POST,$this->session->userdata('supplier_id'));
        ...
        foreach ($products['data'] as $row) {
            ...
            $rowData[] = $row['paid_amount'];
            $rowData[] = $row['pending_amount'];
            $rowData[] = $row['grand_total'];

            $invoice_status = '';
            if( (int)$row['invoice_status'] == 1 ){ $invoice_status = 'Advance Payment'; }
            else if( (int)$row['invoice_status'] == 2 ){ $invoice_status = 'Paid'; }
