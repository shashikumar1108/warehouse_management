...
<body>
          ...
          <div class="template-demo">
            <nav aria-label="breadcrumb">
              <ol class="breadcrumb breadcrumb-custom bg-inverse-primary">
                ...
                <li class="breadcrumb-item active" aria-current="page"><span>Approve Quotation</span></li>
              </ol>
            </nav>
          </div>
          <div class="card">
            ...
            <?php if($this->session->flashdata('status') == 'success' || $this->session->flashdata('status') == 'fail') {
              ...
              <div class="row">
                <div class="col-12 table-responsive">
                  <table id="approve-listing" class="table" style="width:100%">
                    <thead>
                      <tr>
                        <th>Sl.No.</th>                                                
                        <th>Reference No</th>
                    ...
                    <tbody>
                      ...
                      <!-- <?php $i=1; foreach($quotation_list as $row) {
                        ...
                        <tr>
                            ...
                            <td>
                              <button class="btn btn-primary" onclick="viewQ(<?=$row['quotation_id']?>)">View & Approve</button>
                              <?php
                              /*
                              <form method="post" action="<?php echo base_url('orders/approve_quotation') ?>">
    ...
    <div class="modal-dialog modal-lg" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="requestModalLabel">Approve RFQ</h5>
  ...
  function viewQ(id){
    let output='';
    $.ajax({
      url: '<?=base_url("orders/getRequestData")?>',
      type: 'GET',
      data: {'request_id' : id },
      cache: false,
      success : function(result){
        $('#requestModal').modal('show');
        $('#requestModal .modal-body').html(result);
      }
    });
  }
  ...
  <script>
    ...
    function initializeTable(){
        ...
        datatable = $('#approve-listing').DataTable({
            "processing": true,
            "serverSide": true,
            "responsive": true,
            "paging": true,
            ...
            "lengthChange": true,
            "ajax": {
                "url": "<?=base_url()?>orders/getApproveTableList",
                "type": "post",
            },
            // "aLengthMenu": [
            //     [5, 10, 15],
