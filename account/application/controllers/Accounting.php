...
class Accounting extends CI_Controller {
    ...
    public function getAdvancePaymentTableList(){
        ...
        foreach ($products['data'] as $row) {
            ...
            $rowData[] = $row['supplier_name'];
            $rowData[] = '<a href="'.base_url().'accounting/add_advance_payment/'.$row['quotation_id'].'/'.$row['vendor_quote_id'].'">New Advance Payment</a>';
            $i++;
            $data[] = $rowData;
        }
    ...
    function add_advance_payment($quotation_id = 0,$vendor_quote_id = 0){
        if( $quotation_id != 0 && $vendor_quote_id != 0 ){
            $proposal_status = '3';
            ...
            redirect('accounting/advance_payments');
        }        
    }
    // End Advance Payment
    ...
    public function getPendingInvoiceTableList(){
        ...
        foreach ($products['data'] as $row) {
            ...
            $rowData[] = $row['ref_number'];
            $rowData[] = $row['warehouse_name'];
            $rowData[] = $row['supplier_name'].'<br>'.$row['phone'].'<br>'.$row['email'];
            $rowData[] = '<a href="'.base_url().'accounting/clear_pending_invoice/'.$row['quotation_id'].'/'.$row['vendor_quote_id'].'">Clear Payment</a>';
            $i++;
            $data[] = $rowData;
        }
    ...
    public function getPaidInvoiceTableList(){
        ...
        foreach ($products['data'] as $row) {
            ...
            $rowData[] = $row['warehouse_name'];
            $rowData[] = $row['supplier_name'].'<br>'.$row['phone'].'<br>'.$row['email'];
            $rowData[] = '<a href="'.base_url().'accounting/paid_invoice_details/'.$row['quotation_id'].'/'.$row['vendor_quote_id'].'">View Details</a>';
            $i++;
            $data[] = $rowData;
        }
