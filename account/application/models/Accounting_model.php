...
class Accounting_model extends CI_Model {
   ...
   public function get_quotation_invoice_details($proposal_status,$invoice_status, $vendor_quote_id = 0){
   ...
   public function get_quotation_invoice_details_table_list($payload = array(), $proposal_status = 4, $invoice_status = 0){
      ...
      $this->db->select($select)
         ...
         ->join('vendor_quotation vq','q.quotation_id=vq.quotation_id and i.vendor_quote_id = vq.vendor_quote_id')
   ...
   public function get_quotation_invoice($quotation_id,$invoice_status,$vendor_quote_id = 0){
      $this->db->select('i.*');
      $this->db->from('quotation q');
      $this->db->join('invoice i','q.quotation_id=i.quotation_id');
      $this->db->where(array('i.quotation_id' => $quotation_id,'invoice_status' => $invoice_status));
      if( $vendor_quote_id != 0 ){
         $this->db->where('i.vendor_quote_id',$vendor_quote_id);
      }
