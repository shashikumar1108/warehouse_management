...
class Vendor extends CI_Controller {
    ...
    public function vendorQuoteTableData($payload = array()){
        ...
        $this->db->select('q.quotation_id')
            ->from('quotation q')
            ->join('warehouse w', 'w.id = q.warehouse_id')
            ->where(array('q.delete_status'=>0,'q.approval_status'=>'approved','q.quotation_status'=>1));
    ...
    public function getConfirmedDeliveryNoteTableList(){
        ...
        foreach ($records['data'] as $row) {
            ...
            $rowData[] = $row['ref_number'];
            $rowData[] = $row['warehouse'];
            $rowData[] = "<a class='btn btn-primary' href='".base_url('vendor/add_delivery_note/'.$row['quotation_id'])."'>New Note</a>";
            $i++;
            $data[] = $rowData;
        }
    ...
    public function add_delivery_note($quotation_id){
        $proposal_status = '2';
        $data['vendor_quotation'] = $this->vendor_model->get_single_vendor_quotation_details($quotation_id,$proposal_status);
        $data['quotation_price_details'] = $this->vendor_model->get_vendor_proposed_price_details($quotation_id,$proposal_status);
        // echo "<pre>";
