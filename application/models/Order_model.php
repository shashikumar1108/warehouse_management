...
class Order_model extends CI_Model {
   ...
   public function getWarehouseSuppliers($warehouse_id = null)
   {
      ...
      public function get_order_price_details($quotation_id,$proposal_status, $vendor_quote_id = 0){
         ...
         $this->db->join('products p','qp.product_id=p.id');
         $this->db->where(array('q.quotation_id' => $quotation_id,'vq.proposal_status' => $proposal_status,'vqad.status'=>1));
         // $this->db->where(array('q.quotation_id' => $quotation_id));
         if( $vendor_quote_id != 0 ){
            $this->db->where(array('vq.vendor_quote_id' => $vendor_quote_id,'vqad.vendor_quote_id'=>$vendor_quote_id));
         }
         return $this->db->get()->result_array();
      }
