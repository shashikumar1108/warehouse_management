        ...
        <div class="content-wrapper">
          <div class="card">
            ...
            || $this->session->flashdata('status') == 'fail') {
              ...
              <span class="card-title">Towns</span>
              <span>
                    ...
                    <?php $i = 0;                    
                        foreach($data as $d){ $i++;    ?>
                            <tr>
                                <td><?php echo $i ?></td>
                                <td>
                                <label class="badge badge-danger"><a href="<?php echo base_url(); ?>Initilization/towns/delete?id=<?php echo $d['id'] ?>">Delete</a></label>
                  ...
                  <div class="modal fade" id="edit_<?php echo $d['id']; ?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel-4" aria-hidden="true">
                    <div class="modal-dialog" role="document">
                      <div class="modal-content">
                        ...
                        <form method="post" action="<?php echo base_url('Initilization/towns/edit'); ?>">
                            <input type="hidden" name="id" id="id" value="<?php echo $d['id']; ?>" />
                            
                            <?php $regions = $this->db->select('*')
                            ->from('regions')
                            ->where('delete_status',0)
                            ->get()->result_array(); ?>

                            <div class="form-group">
                            <label for="name" class="col-form-label">Region:</label><br>
                              <select class="form-control js-example-basic-single w-100%" style="width:100%" name="region_id" id="region_id" required>
                              <option value="">Select Region</option>
                              <?php foreach($regions as $r){ ?>
                                <option value="<?php echo $r['id']?>" <?php if($d['region_id'] == $r['id']){echo "selected";} ?> ><?php echo $r['name'] ?></option>
                              <?php } ?>                                                                                                                                                  
                              </select>
                            </div>
                ...
                <div class="modal fade" id="add" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel-4" aria-hidden="true">
                    <div class="modal-dialog" role="document">
                      <div class="modal-content">
                        ...
                        <div class="modal-body">
                          <form method="post" action="<?php echo base_url('Initilization/towns/add'); ?>">
